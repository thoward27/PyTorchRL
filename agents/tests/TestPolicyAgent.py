...
from agents.PolicyAgent import *
...
class TestAgent(TestCase):
    def test_init(self):
        a = PolicyAgent(in_dim=1, out_dim=4)
        self.assertIsInstance(a, PolicyAgent)
        return

    def test_forward(self):
        a = PolicyAgent(in_dim=1, out_dim=1)
    ...
    def test_loss(self):
        b = Bandit([0, 0, 10, 0])
        a = PolicyAgent(in_dim=1, out_dim=len(b))
    ...
    def test_learn(self, arms):
        b = Bandit(arms)
        a = PolicyAgent(in_dim=len(b.context), out_dim=len(b))
